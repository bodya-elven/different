(function () {
    ...
    function TMDBKeywords() {
        ...
        this.init = function () {
            ...
            Lampa.Listener.follow('full', function (e) {
                if (e.type == 'complite') {
                    ...
                    if (card && (card.source == 'tmdb' || e.data.source == 'tmdb') && card.id) {
                        // Затримка для черги завантаження
                        setTimeout(function() {
                            // Перевіряємо, чи картка ще активна
                            if (Lampa.Activity.active().activity) {
                                _this.getKeywords(Lampa.Activity.active().activity.render(), card);
                            }
                        }, 800); 
                    }
                }
            ...
            style.innerHTML = '.keywords-icon-svg{width:1.8em;height:1.8em;display:block;pointer-events:none}.button--keywords{display:flex;align-items:center;justify-content:center;gap:0.5em}@media screen and (max-width:768px){.button--keywords{padding:0.5em!important}.keywords-icon-svg{width:1.5em;height:1.5em}}';
        ...
        this.getKeywords = function (html, card) {
            ...
            // === ГОЛОВНЕ ВИПРАВЛЕННЯ API ===
            // Формуємо URL точно так само, як це робить плагін Networks
            // 1. Lampa.TMDB.api() додає правильний домен (проксі)
            // 2. Lampa.TMDB.key() додає ваш API ключ
            var url = Lampa.TMDB.api(method + '/' + card.id + '/keywords?api_key=' + Lampa.TMDB.key());

            // Використовуємо прямий запит через jQuery (найбільш надійний спосіб в Lampa)
            $.ajax({
                url: url,
                dataType: 'json',
                async: true,
                ...
                error: function (xhr, status, error) {
                    console.log('TMDB Keywords: API Request Failed', error);
                }
            });
        };
    ...
    if (!window.plugin_tmdb_keywords_api) {
        window.plugin_tmdb_keywords_api = new TMDBKeywords();
        window.plugin_tmdb_keywords_api.init();
    }
})();
