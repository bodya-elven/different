(function () {
    ...
    function startPlugin() {
        ...
        //======= СТИЛІ (Сітка 16:9 та Porndish) =======//
        var css = '<style>.main-grid { padding: 0 !important; } @media screen and (max-width: 580px) { .main-grid .card { width: 100% !important; margin-bottom: 10px !important; padding: 0 5px !important; } .main-grid.is-categories-grid .card, .main-grid.is-models-grid .card, .main-grid.is-noimg-grid .card, .main-grid.is-16x9-grid .card { width: 50% !important; } } @media screen and (min-width: 581px) { .main-grid .card { width: 25% !important; margin-bottom: 15px !important; padding: 0 8px !important; } .main-grid.is-categories-grid .card, .main-grid.is-models-grid .card, .main-grid.is-noimg-grid .card, .main-grid.is-16x9-grid .card { width: 16.666% !important; } } .main-grid .card__view { padding-bottom: 56.25% !important; border-radius: 12px !important; position: relative !important; } .main-grid.is-categories-grid .card__view { padding-bottom: 80% !important; background: #ffffff !important; } .main-grid.is-models-grid .card__view { padding-bottom: 150% !important; background: #ffffff !important; } .main-grid.is-16x9-grid .card__view { padding-bottom: 56.25% !important; background: #ffffff !important; border-radius: 8px !important; } .main-grid .card__img { object-fit: cover !important; border-radius: 12px !important; position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: 1 !important; } .main-grid.is-16x9-grid .card__img { border-radius: 8px !important; } .main-grid .card__title { display: -webkit-box !important; -webkit-line-clamp: 3 !important; -webkit-box-orient: vertical !important; overflow: hidden !important; white-space: normal !important; text-align: left !important; line-height: 1.2 !important; max-height: 3.6em !important; padding-top: 2px !important; margin-top: 0 !important; text-overflow: ellipsis !important; } .main-grid.is-categories-grid .card__title, .main-grid.is-models-grid .card__title, .main-grid.is-16x9-grid .card__title { -webkit-line-clamp: 2 !important; text-align: center !important; font-weight: normal !important; margin-top: 5px !important; } .main-grid.is-noimg-grid .card { position: relative !important; } .main-grid.is-noimg-grid .card__view { padding-bottom: 25% !important; background: #c4c4c4 !important; border-radius: 8px !important; border: 1px solid #aaa; transition: transform 0.2s; } .main-grid.is-noimg-grid .card.focus .card__view { transform: scale(1.05); background: #b0b0b0 !important; border-color: #fff; box-shadow: 0 0 10px rgba(255,255,255,0.8); } .main-grid.is-noimg-grid .card__img { display: none !important; } .main-grid.is-noimg-grid .card__title { position: absolute !important; top: 0; left: 0; width: 100%; height: 100%; display: flex !important; align-items: center !important; justify-content: center !important; color: #000000 !important; font-weight: bold !important; font-size: 1.3em !important; line-height: 1.2 !important; text-align: center !important; white-space: normal !important; word-break: break-word !important; -webkit-line-clamp: unset !important; padding: 8px !important; margin: 0 !important; z-index: 10; } .main-grid.is-porndish-list .card { width: 100% !important; margin-bottom: 10px !important; padding: 0 10px !important; height: auto !important; } .main-grid.is-porndish-list .card__view { display: none !important; } .main-grid.is-porndish-list .card__title { position: relative !important; width: 100% !important; height: 100% !important; color: #ffffff !important; font-weight: normal !important; font-size: 1.3em !important; text-align: center !important; display: flex !important; align-items: center !important; justify-content: center !important; white-space: normal !important; padding: 15px 15px !important; margin: 0 !important; background: rgba(35,35,35,0.9) !important; border-radius: 8px !important; border: 1px solid rgba(255,255,255,0.1) !important; transition: transform 0.2s !important; } .main-grid.is-porndish-list .card.focus .card__title { transform: scale(1.01) !important; background: rgba(70,70,70,0.95) !important; border-color: #fff !important; z-index: 10 !important; } @media screen and (min-width: 581px) { .main-grid.is-porndish-list .card { width: 50% !important; } .main-grid.is-porndish-list .card__title { font-size: 1.4em !important; } } .main-grid .card__age, .main-grid .card__textbox { display: none !important; } .pluginx-filter-btn { order: -1 !important; margin-right: auto !important; }</style>'; $('body').append(css);
        ...
        Adapters['porno365'] = {
            ...
            parse: function(doc, obj) {
                var sel = obj.is_related ? '.related .related_video' : 'li.video_block, li.trailer', elements = doc.querySelectorAll(sel), results = [];
                var targetPath = (obj.url || '').replace(this.domain, '').split('?')[0];
                if (targetPath === '/categories' || obj.is_categories) { var links = doc.querySelectorAll('.categories-list-div a'), added = []; for (var i = 0; i < links.length; i++) { var el = links[i], title = (el.getAttribute('title') || el.textContent || '').trim(), href = el.getAttribute('href'); if (!title) continue; var imgEl = el.querySelector('img'), img = imgEl ? (imgEl.getAttribute('data-src') || imgEl.getAttribute('src')) : ''; if (img && img.indexOf('//') === 0) img = 'https:' + img; if (href && title && added.indexOf(href) === -1) { results.push({ name: title, url: this.domain + href, picture: img, img: img, is_grid: true }); added.push(href); } } return results; }
                if (obj.is_models || targetPath === '/models' || targetPath.indexOf('/models/sort-by-') === 0) { var mEls = doc.querySelectorAll('.item_model'); for (var k = 0; k < mEls.length; k++) { var elM = mEls[k], linkM = elM.querySelector('a'), nameM = elM.querySelector('.model_eng_name'), countM = elM.querySelector('.cnt_span'), imgM = elM.querySelector('img'); if (linkM && nameM) results.push({ name: formatTitle((nameM.textContent || '').trim(), countM ? (countM.textContent || '').trim() : '', '☰'), url: this.domain + linkM.getAttribute('href'), picture: imgM ? imgM.getAttribute('src') : '', img: imgM ? imgM.getAttribute('src') : '', is_grid: true, is_models_grid: true }); } return results; }
                for (var j = 0; j < elements.length; j++) { var el2 = elements[j], linkEl2 = el2.querySelector('a.image'), titleEl = el2.querySelector('a.image p, .title'), imgEl2 = el2.querySelector('img'), timeEl = el2.querySelector('.duration'); if (linkEl2 && titleEl) { var img2 = imgEl2 ? (imgEl2.getAttribute('data-src') || imgEl2.getAttribute('data-original') || imgEl2.getAttribute('src')) : ''; if (img2 && img2.indexOf('//') === 0) img2 = 'https:' + img2; var vUrl = this.domain + linkEl2.getAttribute('href'); var name = (titleEl.textContent || '').trim(), time = timeEl ? (timeEl.textContent || '').trim() : ''; results.push({ name: formatTitle(name, time, '▶'), url: vUrl, picture: img2, img: img2 }); } } return results;
            },
            ...
            getSort: function(doc) {
                ...
                if (stext) { var divs = stext.querySelectorAll('.div_sort'); for (var d = 0; d < divs.length; d++) { var span = divs[d].querySelector('span'), prefix = span ? span.textContent.replace(/:$/, '').trim() + ' - ' : '', links = divs[d].querySelectorAll('a, span.active_sort, strong'); for (var l = 0; l < links.length; l++) { var text = links[l].textContent.trim(); if(text === 'Сортування') continue; var fullTitle = prefix + text; if (links[l].classList.contains('active_sort') || links[l].tagName === 'STRONG') { activeTitle = fullTitle; items.push({ title: '⇅ ' + fullTitle, action: 'none' }); } else if(links[l].tagName === 'A' && links[l].getAttribute('href').indexOf('/ru') === -1) { items.push({ title: fullTitle, url: this.domain + links[l].getAttribute('href') }); } } } }
                return { active: activeTitle, items: items };
            },
            getMenu: function(doc) { var m = []; var mEls = doc.querySelectorAll('.video-categories.video-models a'); for (var i = 0; i < mEls.length; i++) m.push({ title: (mEls[i].textContent || '').trim(), action: 'direct', url: mEls[i].getAttribute('href') }); m.push({ title: 'Категорії', action: 'cats_custom', sel: '.video-categories:not(.video-models) a' }, { title: 'Схожі відео', action: 'sim' }); return m; },
            play: function(doc, el) { var str = [], q = doc.querySelectorAll('.quality_chooser a'); for (var j = q.length - 1; j >= 0; j--) { var qHref = q[j].getAttribute('href'); if(qHref) str.push({ title: (q[j].textContent || '').trim(), url: qHref }); } if (str.length > 0) { Lampa.Player.play({ title: el.name, url: str[0].url, quality: str }); Lampa.Player.playlist([{ title: el.name, url: str[0].url, quality: str }]); } }
        };
        ...
        Adapters['lenkino'] = {
            ...
            parse: function(doc, obj) {
                var targetPath = (obj.url || '').replace(this.domain, '').split('?')[0];
                if (targetPath === '/categories' || obj.is_categories) { var resCat = [], cats = doc.querySelectorAll('.grd-cat a'); for(var c=0; c<cats.length; c++) { var tC = cats[c].getAttribute('title') || cats[c].textContent; var iC = cats[c].querySelector('img'); var sC = iC ? (iC.getAttribute('data-src')||iC.getAttribute('src')) : ''; var hC = cats[c].getAttribute('href'); if(tC && hC) resCat.push({name: tC.trim(), url: hC.startsWith('http')?hC:this.domain+hC, picture: sC, is_grid: true, is_16x9_grid: true}); } return resCat; }
                if (targetPath === '/pornstars' || obj.is_models) { var resM = [], mdls = doc.querySelectorAll('.grd-mdl a'); for(var m=0; m<mdls.length; m++) { var iM = mdls[m].querySelector('img'), tM = mdls[m].querySelector('.itm-tit'), cM = mdls[m].querySelector('.itm-opt li'), nC = tM ? tM.textContent : (iM?iM.getAttribute('alt'):''), sM = iM ? (iM.getAttribute('data-src')||iM.getAttribute('src')):''; if(nC) resM.push({name: formatTitle(nC.trim(), cM?cM.textContent:'', '☰'), url: this.domain+mdls[m].getAttribute('href'), picture: sM, is_grid: true, is_16x9_grid: true}); } return resM; }
                var isStudios = obj.is_studios || targetPath.indexOf('/channels') !== -1, res = [], els = doc.querySelectorAll(isStudios ? '.itm-crd-spn, .itm-crd' : '.item:not(.sxn-top .item)'); for(var i=0; i<els.length; i++) { var link = els[i].querySelector(isStudios ? 'a.len_pucl' : 'a'), tit = els[i].querySelector(isStudios ? '.itm-opt' : '.itm-tit'), img = els[i].querySelector('img'), dur = els[i].querySelector(isStudios ? '.itm-opt li' : '.itm-dur'); if(link) { var name = isStudios ? (link.getAttribute('title')||tit.textContent) : (tit?tit.textContent:link.textContent), src = img ? (img.getAttribute('data-srcset')||img.getAttribute('data-src')||img.getAttribute('src')) : ''; if(name) res.push({name: formatTitle(name.trim(), dur?dur.textContent:'', isStudios?'☰':'▶'), url: this.domain+link.getAttribute('href'), picture: src, is_grid: isStudios}); } } return res;
            },
            getFilter: function() { return [ { title: '🗄️ Категорії', action: 'categories' }, { title: '👸 Моделі', action: 'models' }, { title: '🎬 Студії', action: 'studios' } ]; },
            getSort: function(doc) { var act = 'Новое', items = [], btns = doc.querySelector('.tabs .btns.btns-s'); if (btns) { var a = btns.querySelector('.act'); if(a) act = a.textContent.trim(); var links = btns.querySelectorAll('a'); for (var i=0; i<links.length; i++) { var t = links[i].textContent.trim(), h = links[i].getAttribute('href'); if(t===act) items.push({title: '⇅ '+t, action:'none'}); else items.push({title: t, url: this.domain+(h.startsWith('/')?'':'/')+h}); } } return { active: act, items: items }; },
            getMenu: function(doc) { var m = [], mEls = doc.querySelectorAll('.grd-mdl a'); for (var i = 0; i < mEls.length; i++) m.push({ title: (mEls[i].textContent || '').trim(), action: 'direct', url: mEls[i].getAttribute('href') }); m.push({ title: 'Категорії', action: 'cats_custom', sel: '.vid-cat a' }, { title: 'Схожі відео', action: 'sim' }); return m; },
            play: function(doc, el) { var str = [], a = doc.documentElement.innerHTML.match(/video_alt_url:[\t ]+'([^']+)'/), u = doc.documentElement.innerHTML.match(/video_url:[\t ]+'([^']+)'/); if (a) str.push({ title: 'HD', url: a[1] }); if (u) str.push({ title: 'SD', url: u[1] }); if (str.length > 0) { var pData = { title: el.name, url: str[0].url, quality: str, headers: { 'Referer': this.domain+'/', 'Origin': this.domain } }; Lampa.Player.play(pData); Lampa.Player.playlist([pData]); } }
        };
        //======= PORNMZ =======//
        Adapters['pornmz'] = {
            ...
            parse: function(doc, obj) {
                var targetPath = (obj.url || '').replace(this.domain, '').split('?')[0];
                var isGrid = obj.is_categories || obj.is_models || targetPath.indexOf('/categories') !== -1 || targetPath.indexOf('/actors') !== -1;
                var res = [], elements = doc.querySelectorAll('.videos-list article, article[data-video-id], article.thumb-block');
                for (var i = 0; i < elements.length; i++) {
                    var el = elements[i], linkEl = el.querySelector('a'), titleSpan = el.querySelector('.title, .entry-title, .cat-title');
                    if (linkEl) {
                        ...
                        var name = titleSpan ? (titleSpan.textContent || '').replace(/\d+[KM]*$/, '').trim() : linkEl.getAttribute('title') || '';
            ...
            play: function(doc, el) { 
                ...
                } else if (vSrc) { Lampa.Player.play({ title: el.name, url: vSrc }); Lampa.Player.playlist([{ title: el.name, url: vSrc }]); }
            },
        ...
        Adapters['pornhub'] = {
            ...
            parse: function(doc, obj) {
                ...
                if (targetPath === '/pornstars' || targetPath === '/channels' || obj.is_models) { var isSt = targetPath === '/channels', resM = [], sel = isSt ? '.channelsList li' : '#pornstarListSection li', mEls = doc.querySelectorAll(sel); for(var m=0; m<mEls.length; m++) { var lM = mEls[m].querySelector('a'), iM = mEls[m].querySelector('img'), tM = mEls[m].querySelector('.title'); if(lM && iM) { var nM = tM ? tM.textContent.trim() : iM.getAttribute('alt'); if(nM) resM.push({name: formatTitle(nM, '', '☰'), url: this.domain+lM.getAttribute('href'), picture: iM.getAttribute('src'), is_grid: true, is_models_grid: !isSt, is_studios_noimg: isSt}); } } return resM; }
                var res = [], elements = doc.querySelectorAll('li.videoblock'); for (var i = 0; i < elements.length; i++) { var el = elements[i]; if (el.className.indexOf('marker-next-videos') !== -1) continue; var linkEl = el.querySelector('a.linkVideoThumb'), imgEl = el.querySelector('img'), timeEl = el.querySelector('.duration'); if (linkEl && imgEl) { var name = imgEl.getAttribute('title') || imgEl.getAttribute('alt') || ''; var vUrl = linkEl.getAttribute('href'); if (vUrl && vUrl.indexOf('javascript') === -1) { var img = imgEl.getAttribute('src') || ''; var time = timeEl ? (timeEl.textContent || '').trim() : ''; res.push({ name: formatTitle(name, time, '▶'), url: this.domain + vUrl, picture: img }); } } } return res;
            },
            ...
            getMenu: function(doc) { var m = [], phModels = doc.querySelectorAll('.pornstarsWrapper .pstar-list-btn'); for (var p = 0; p < phModels.length; p++) m.push({ title: (phModels[p].textContent || '').trim(), action: 'direct', url: this.domain + phModels[p].getAttribute('href') }); m.push({ title: 'Схожі відео', action: 'sim' }); return m; },
        ...
        Adapters['porndish'] = {
            ...
            parse: function(doc, obj) {
                var targetPath = (obj.url || '').replace(this.domain, '').split('?')[0];
                if (targetPath === '/categories' || obj.is_categories) { var resCat = [], cats = doc.querySelectorAll('.g1-nav-menu li a'); for(var c=0; c<cats.length; c++) { var tC = cats[c].textContent.trim(), hC = cats[c].getAttribute('href'); if(tC && hC) resCat.push({name: tC, url: hC, is_grid: true}); } return resCat; }
                var res = [], elements = doc.querySelectorAll('article.post'); for (var i = 0; i < elements.length; i++) { var el = elements[i], linkEl = el.querySelector('.entry-title a'); if (linkEl) { var vUrl = linkEl.getAttribute('href'), name = linkEl.textContent.trim(), timeEl = el.querySelector('.mace-video-duration'); if (name && vUrl) res.push({ name: formatTitle(name, timeEl?timeEl.textContent.trim():'', '▶'), url: vUrl, is_porndish_list: true }); } } return res;
            },
            getFilter: function() { return [ { title: '🗄️ Категорії', action: 'categories' } ]; },
            getMenu: function() { return [ { title: 'Схожі відео', action: 'sim' } ]; },
            play: function(doc, el) { var streamix = doc.documentElement.innerHTML.match(/streamixContent.*?src=\\?["']([^"'\s>]+)\\?["']/i); if (streamix) { var rawUrl = streamix[1].replace(/\\/g, ''), parts = rawUrl.match(/https?:\/\/([^\/]+)\/.*?(?:e|v|embed|d)\/([a-zA-Z0-9_-]+)/i); if (parts) { smartRequest('https://' + parts[1] + '/api/stream?filecode=' + parts[2], function(j) { var jd = JSON.parse(j); if (jd.streaming_url) Lampa.Player.play({ title: el.name, url: jd.streaming_url }); }, null, { "Referer": "https://" + parts[1] + "/" }); return; } } Lampa.Noty.show('Не знайдено відео'); }
        };

        //======= LONGVIDEOS =======//
        Adapters['longvideos'] = {
            domain: LONGVIDEOS_DOMAIN,
            parse: function(doc, obj) { var res = [], sel = obj.is_related ? '.related-videos .item' : '.list-videos .item', els = doc.querySelectorAll(sel); for(var i=0;i<els.length;i++) { var l = els[i].querySelector('a.thumb_title'); if(l) res.push({name: formatTitle(l.textContent.trim(), '', '▶'), url: this.domain+l.getAttribute('href'), picture: els[i].querySelector('img')?els[i].querySelector('img').getAttribute('src'):''}); } return res; },
            getFilter: function() { return [{ title: '🗄️ Категорії', action: 'lv_cat_list' }, { title: '🔥 Тренди', action: 'lv_trend_list' }]; },
            getMenu: function() { return [{ title: 'Схожі відео', action: 'sim' }]; },
            play: function(doc, el) { var str = [], src = doc.querySelectorAll('video source'); for(var i=0;i<src.length;i++) str.push({title: src[i].getAttribute('label')||'Оригінал', url: src[i].getAttribute('src')}); if(str.length>0) Lampa.Player.play({title: el.name, url: str[0].url, quality: str}); }
        };
        ...
        function CustomCatalog(object) {
            ...
            comp.create = function () {
                ...
                smartRequest(target, function (htmlText) {
                    ...
                    // Перевірка на кінець сторінок (запобіжник)
            ...
            comp.filter = function () {
                ...
                if (this._sortData && this._sortData.items && this._sortData.items.length > 0) {
                    items.push({ title: '↕️ Сортування', subtitle: '⇅ ' + this._sortData.active, action: 'sort' });
                }
                ...
                Lampa.Select.show({ title: 'Навігація', items: items, onSelect: function (a) {
                    ...
                    else if (a.action === 'sort') Lampa.Select.show({ title: 'Сортування', items: comp._sortData.items, onSelect: function(s) { if (s.action === 'none') return; Lampa.Activity.push({ url: s.url, title: s.title.replace('⇅ ', ''), component: 'pluginx_comp', site: currentSite, page: 1, is_models: object.is_models, is_studios: object.is_studios }); }, onBack: function() { comp.filter(); } });
