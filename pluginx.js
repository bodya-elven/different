(function () {
    ...
    // ==========================================
    // ВСТАВТЕ ВАШ ДОМЕН ТУТ (бажано з https://):
    var MY_CATALOG_DOMAIN = 'https://w.porno365.gold'; 
    // ==========================================

    // Безпечна функція для отримання базового URL
    ...
    function startPlugin() {
        ...
        try {
                ...
                comp.create = function () {
                    ...
                    network.silent(object.url || MY_CATALOG_DOMAIN, function (htmlText) {
                        try {
                            ...
                            var siteBaseUrl = getBaseUrl(MY_CATALOG_DOMAIN);
                            ...
                            console.log('Помилка парсингу сторінки', e);
                            _this.empty();
                        }
            ...
            if (window.Lampa && window.Lampa.Noty) {
                window.Lampa.Noty.show('Помилка плагіна: ' + e.message);
            }
        }
    }
    ...
    function ensureMenuExists() {
        try {
            if (typeof $ === 'undefined') return; // Чекаємо, поки jQuery буде готовий
            ...
            console.log('Помилка меню:', e.message);
        }
    }
