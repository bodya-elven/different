...
(function() {
  ...
  */
  ...
  // Вбудовані SVG іконки (поки немає зовнішніх файлів)
  ...
  var RCFG_DEFAULT = {
    ...
    ratings_enable_imdb: true, ratings_enable_tmdb: true, ratings_enable_mc: true,
    ratings_enable_rt: true, ratings_enable_popcorn: true, ratings_enable_trakt: true, ratings_enable_letterboxd: true,
    ratings_order_imdb: 1, ratings_order_tmdb: 2, ratings_order_trakt: 3, 
    ratings_order_letterboxd: 4, ratings_order_rt: 5, ratings_order_popcorn: 6, ratings_order_mc: 7
  };
  ...
  function insertRatings(data) {
    ...
    var sourcesToRender = [
      { id: 'imdb', key: 'imdb', data: data.imdb, enabled: cfg.enIMDB, order: cfg.ordIMDB, icon: ICONS.imdb, title: 'IMDb', class: 'rate--imdb' },
      { id: 'tmdb', key: 'tmdb', data: data.tmdb, enabled: cfg.enTMDB, order: cfg.ordTMDB, icon: ICONS.tmdb, title: 'TMDB', class: 'rate--tmdb' },
      { id: 'trakt', key: 'trakt', data: data.trakt, enabled: cfg.enTrakt, order: cfg.ordTrakt, icon: ICONS.trakt, title: 'Trakt', class: 'rate--trakt' },
      { id: 'letterboxd', key: 'letterboxd', data: data.letterboxd, enabled: cfg.enLetterboxd, order: cfg.ordLetterboxd, icon: ICONS.letterboxd, title: 'Letterboxd', class: 'rate--letterboxd' },
      { id: 'mc', key: 'metacritic', data: data.metacritic, enabled: cfg.enMC, order: cfg.ordMC, icon: ICONS.metacritic, title: 'Metacritic', class: 'rate--mc' },
      { id: 'rt', key: 'rottentomatoes', data: data.rottentomatoes, enabled: cfg.enRT, order: cfg.ordRT, icon: (data.rottentomatoes && data.rottentomatoes.fresh ? ICONS.rotten_good : ICONS.rotten_bad), title: 'Rotten Tomatoes', class: 'rate--rt' },
      { id: 'popcorn', key: 'popcorn', data: data.popcorn, enabled: cfg.enPopcorn, order: cfg.ordPopcorn, icon: ICONS.popcorn, title: 'Popcornmeter', class: 'rate--popcorn' }
    ];

    $('.rate--imdb:not(.lmp-custom-rate), .rate--tmdb:not(.lmp-custom-rate)', rateLine).addClass('hide').hide();

    sourcesToRender.sort(function(a, b) { return a.order - b.order; });

    sourcesToRender.forEach(function(src) {
      if (!src.enabled || !src.data) return;

      var extraStyle = (src.id === 'rt' && src.data.fresh) ? 'border-radius:4px;' : '';
      var colorClass = cfg.colorizeAll ? getRatingClass(src.data.avg) : '';

      var cont = $(
        '<div class="full-start__rate lmp-custom-rate ' + src.class + ' ' + colorClass + '">' +
        '<div>' + src.data.display + '</div>' +
        '<div class="source--name" title="' + src.title + '">' + iconImg(src.icon, src.title, 22, extraStyle) + '</div>' +
        '</div>'
      );
  ...
  */
  function getCfg() {
    ...
    return {
      ...
      enIMDB: !!Lampa.Storage.field('ratings_enable_imdb', RCFG_DEFAULT.ratings_enable_imdb),
      enTMDB: !!Lampa.Storage.field('ratings_enable_tmdb', RCFG_DEFAULT.ratings_enable_tmdb),
      enMC: !!Lampa.Storage.field('ratings_enable_mc', RCFG_DEFAULT.ratings_enable_mc),
      enRT: !!Lampa.Storage.field('ratings_enable_rt', RCFG_DEFAULT.ratings_enable_rt),
      enPopcorn: !!Lampa.Storage.field('ratings_enable_popcorn', RCFG_DEFAULT.ratings_enable_popcorn),
      enTrakt: !!Lampa.Storage.field('ratings_enable_trakt', RCFG_DEFAULT.ratings_enable_trakt),
      enLetterboxd: !!Lampa.Storage.field('ratings_enable_letterboxd', RCFG_DEFAULT.ratings_enable_letterboxd),

      ordIMDB: parseIntDef('ratings_order_imdb', RCFG_DEFAULT.ratings_order_imdb),
      ordTMDB: parseIntDef('ratings_order_tmdb', RCFG_DEFAULT.ratings_order_tmdb),
      ordTrakt: parseIntDef('ratings_order_trakt', RCFG_DEFAULT.ratings_order_trakt),
      ordLetterboxd: parseIntDef('ratings_order_letterboxd', RCFG_DEFAULT.ratings_order_letterboxd),
      ordRT: parseIntDef('ratings_order_rt', RCFG_DEFAULT.ratings_order_rt),
      ordPopcorn: parseIntDef('ratings_order_popcorn', RCFG_DEFAULT.ratings_order_popcorn),
      ordMC: parseIntDef('ratings_order_mc', RCFG_DEFAULT.ratings_order_mc)
    };
  }
  ...
  function addSettingsSection() {
    ...
    var sources = [
      ...
      { id: 'letterboxd', name: 'Letterboxd' }, { id: 'rt', name: 'RottenTomatoes' }, 
      { id: 'popcorn', name: 'Popcornmeter' }, { id: 'mc', name: 'Metacritic' }
    ];
