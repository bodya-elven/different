(function () {
    ...
    // 1. Завантажуємо ідеальний фон (ТБ - бекдроп, Телефон - постер без тексту)
    function loadOriginalPoster(e, render) {
        ...
        if (data) {
            if (isMobile) {
                ...
                // Пріоритет для мобільного: Чистий постер -> Бекдроп -> Звичайний постер
                tmdbPath = textlessPoster || data.backdrop_path || data.poster_path;
            } else {
                // Для ТБ (широкі екрани) завжди беремо бекдроп
        ...
        if (tmdbPath) {
            ...
            // Видаляємо старий кастомний фон
            render.find('.cardify-custom-bg').remove();
            
            // Створюємо новий елемент фону
            ...
            // Додаємо його у контейнер
            render.prepend(customBg);
        }
    }

    // 2. Обробка UI (чистка рядків, застосування налаштувань)
    ...
    function initTemplatesAndStyles() {
        // Шаблон сторінки. Додано клас full-start__buttons для сумісності з іншими плагінами!
        Lampa.Template.add(
            "full_start_new",
