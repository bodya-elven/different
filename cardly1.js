(function () {
    ...
    // Завантаження фонового зображення (Адаптація: Портрет для мобільних, Пейзаж для ТБ)
    function loadOriginalPoster(e, render) {
        ...
        if (data) {
            // Для телефонів беремо вертикальний постер, для ТБ - горизонтальний бекдроп
            if (isMobile) {
                tmdbPath = data.poster_path || data.backdrop_path;
            } else {
                tmdbPath = data.backdrop_path || data.poster_path;
            }
        } else if (bgImg.attr("src")) {
            var srcMatch = bgImg.attr("src").match(/\/([^\/]+\.jpg)$/);
            if (srcMatch) tmdbPath = "/" + srcMatch[1];
        }
    ...
    // Застосування візуальних змін до сторінки фільму
    function applyCardifyUI(e) {
        ...
        // Очищення відображення сезонів/серій
        ...
        // Застосування налаштувань (рейтинг, статус)
        ...
        // Встановлюємо правильний постер (горизонтальний або вертикальний)
        loadOriginalPoster(e, render);

        // Підвантаження додаткового контенту Lampa, якщо він є
    ...
    // Ініціалізація HTML та CSS
    function initTemplatesAndStyles() {
        // Шаблон сторінки (повністю ідентичний оригіналу, але без трейлерних блоків)
        Lampa.Template.add(
            "full_start_new",
    ...
    function startPlugin() {
        ...
        // Перехоплюємо створення сторінки
