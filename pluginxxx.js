    ...
    .pluginx-filter-btn { order: -1 !important; margin-right: auto !important; }\
        ...
        var Adapters = {
            ...
            familyporn: {
                ...
                getUrl: function(object, page) {
                    ...
                    // Обробка пагінації для WordPress (додавання /page/X/)
                    if (page > 1) {
                ...
                getFilters: function(doc, currentUrl) {
                    ...
                    // Каталоги списків (без фільтрації)
                    if (targetPath === '/categories' || targetPath === '/channels' || targetPath === '/pornstars') return null;

                    var isMain = (targetPath === '' || targetPath === '/');
                    
                    if (isMain) {
                        // Сортування для Головної сторінки
                        ...
                        // Сортування для внутрішніх сторінок (Категорії, Студії, Моделі)
                        var activeTitle = 'Latest';
                        if (currentUrl.indexOf('m_sort=views') !== -1) activeTitle = 'Most Viewed';
                        else if (currentUrl.indexOf('m_sort=likes') !== -1) activeTitle = 'Oldest'; // WordPress сортування за датою назад

                        var baseUrl = currentUrl.split('?')[0].replace(/\/page\/[0-9]+\/?$/, '').replace(/\/+$/, '');

                        return [{
                            subtitle: '↕️ ' + activeTitle,
                            items: [
                                { title: 'Latest', url: baseUrl + '/' },
                                { title: 'Oldest', url: baseUrl + '/?m_sort=likes' },
                                { title: 'Most Viewed', url: baseUrl + '/?m_sort=views' }
                            ]
                        }];
                    }
                },
                ...
                getNavItems: function() {
                    return [
                        { title: 'Channels', action: 'nav', url: this.domain + '/channels/' },
                        { title: 'Categories', action: 'nav', url: this.domain + '/categories/' },
                        { title: 'Pornstars', action: 'nav', url: this.domain + '/pornstars/' }
                    ];
                },
                ...
                parse: function(doc, currentUrl, object) {
                    ...
                    // 1. ПАРСИНГ КАТАЛОГУ СТУДІЙ (CHANNELS)
                    if (targetPath === '/channels') {
                        var items = doc.querySelectorAll('.g1-layer-content, .g1-card');
                        for (var i = 0; i < items.length; i++) {
                            var a = items[i].querySelector('h3 a, a.g1-frame');
                            var imgEl = items[i].querySelector('img');
                            var countEl = items[i].querySelector('.g1-meta-entries, p');
                            
                            if (a) {
                                var url = a.getAttribute('href');
                                var title = a.textContent.trim() || (imgEl ? imgEl.getAttribute('alt') : '');
                                var img = imgEl ? (imgEl.getAttribute('src') || imgEl.getAttribute('data-src')) : '';
                                var badge = '';
                                
                                if (countEl) {
                                    var countMatch = countEl.textContent.match(/([0-9]+)\s*entries/i);
                                ...
                                if (url && title) {
                                    results.push({
                                        ...
                                        is_models: false,
                                        is_grid: true,
                                        card_grid: 'categories-grid studios-grid'
                                    });
                                }
                            }
                        }
                    ...
                    // 2. ПАРСИНГ КАТАЛОГУ КАТЕГОРІЙ АБО МОДЕЛЕЙ (ТЕКСТОВІ СПИСКИ)
                    if (targetPath === '/categories' || targetPath === '/pornstars') {
                        var isModelPage = (targetPath === '/pornstars');
                        var items = doc.querySelectorAll('.entry-content p a, .entry-content li a, .g1-column a');
                        
                        // Якщо на сторінці моделей пустий масив, шукаємо ширші блоки
                        if (items.length === 0) items = doc.querySelectorAll('.entry-content a');
                        ...
                        for (var i = 0; i < items.length; i++) {
                            ...
                            if (url && text && text.length > 1) {
                                var title = text.replace(/[0-9]+\s*entries/i, '').replace(/\s*\(.*\)/, '').trim();
                                var badge = '';
                                var countMatch = text.match(/([0-9]+)\s*entries/i);
                                if (countMatch) badge = '🎬 ' + countMatch[1];
                    ...
                    // 3. СТАНДАРТНИЙ ПАРСИНГ КАРТОК ВІДЕО
                    var items = doc.querySelectorAll('li.g1-collection-item, article.g1-card');
                    for (var i = 0; i < items.length; i++) {
                        var item = items[i];
                        var anchor = item.querySelector('h3 a, .g1-frame, article a');
                        ...
                        if (anchor) {
                            ...
                            if (url && title && url.indexOf('/channels/') === -1 && url.indexOf('/categories/') === -1) {
                                if (url.indexOf('http') === -1) url = this.domain + (url.startsWith('/') ? '' : '/') + url.replace(/^\//, '');
                                if (img && img.indexOf('http') === -1) img = 'https:' + img.replace(/^https?:/, '');
                ...
                getMenu: function(doc, htmlText, element) {
                    ...
                    // 1. Моделі та Студія (з блоку медіа-даних під плеєром)
                    var links = doc.querySelectorAll('.g1-meta-categories a, .entry-categories a, .entry-tags a');
                    for (var i = 0; i < links.length; i++) {
                        var aUrl = links[i].getAttribute('href');
                        var aTitle = links[i].textContent.trim();
                        
                        if (aUrl && aTitle && aTitle !== element.name) {
                            if (aUrl.indexOf('http') === -1) aUrl = this.domain + (aUrl.startsWith('/') ? '' : '/') + aUrl.replace(/^\//, '');
                            
                            if (aUrl.indexOf('/pornstar/') !== -1 || aUrl.indexOf('/tag/') !== -1) {
                                // Пряме виведення імені моделі (без префіксів)
                                menu.push({ title: aTitle, action: 'direct', url: aUrl, is_models: true });
                            } else if (aUrl.indexOf('/category/') !== -1) {
                                menu.push({ title: 'Студія: ' + aTitle, action: 'direct', url: aUrl, is_studios: true });
                            }
                        }
                    }

                    // 2. Випадаючий список категорій (збираємо всі інші теги)
                    var catsExist = doc.querySelector('.entry-tags a, .g1-meta-categories a');
                    if (catsExist && !element.is_models) {
                        menu.push({ title: 'Категорії', action: 'cats_custom', sel: '.entry-tags a, .g1-meta-categories a' });
                    }

                    // 3. Схожі відео (блок під плеєром)
                    var simExist = doc.querySelectorAll('aside.g1-related-entries li');
                    if (simExist.length > 0 && !element.is_models) {
        ...
        vtrahe: {
            ...
            getUrl: function(object, page) {
                ...
                if (page > 1) {
                    if (url.indexOf('?story=') !== -1) {
                        return url; // Пагінація пошуку зазвичай працює через POST, залишаємо базовий URL
                    } else {
                        var base = url.split('?')[0].replace(/\/page\/[0-9]+\/?$/, '').replace(/\/+$/, '');
            ...
            checkPagination: function(doc, page) {
                var nav = doc.querySelector('.navigation');
                if (!nav) return false;
                var current = nav.querySelector('span:not(.nav_ext)');
                // Якщо сервер віддав сторінку, номер якої не збігається із запитаною - зупиняємо пагінацію
                if (current && parseInt(current.textContent.trim()) !== page) return false;
                return true;
            },
            ...
            getFilters: function(doc, currentUrl) {
                ...
                if (isModelsList) {
                    ...
                    else if (targetPath.indexOf('sort-by-alphabetical') !== -1) modelTitle = 'По алфавиту';

                    return [{
                        subtitle: '↕️ ' + modelTitle,
                        items: [
                            { title: 'Новые', url: this.domain + '/pornstar/' },
                            { title: 'По популярности', url: this.domain + '/pornstar/sort-by-subscribers/' },
                            { title: 'По количеству', url: this.domain + '/pornstar/sort-by-total/' },
                            { title: 'По алфавиту', url: this.domain + '/pornstar/sort-by-alphabetical/' }
                        ]
                    }];
                }
            ...
            parse: function(doc, currentUrl, object) {
                ...
                if (object.is_related) {
                    ...
                    if (previewBlock && previewBlock.parentNode) container = previewBlock.parentNode;
                }
                
                var items = container.querySelectorAll('.innercont, .pornstar, .categ, .pornstudios a');
                
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    var isVideo = item.classList.contains('innercont');
                    var isModel = item.classList.contains('pornstar');
                    var isCategory = item.classList.contains('categ');
                    var isStudio = item.tagName && item.tagName.toLowerCase() === 'a' && item.parentElement && item.parentElement.classList.contains('pornstudios');
                    
                    var url = '', img = '', title = '', time = '', badge = '', previewUrl = '';
                    ...
                    if (isVideo) {
                        ...
                        var imgEl = item.querySelector('img');
                        var titleEl = item.querySelector('.pornstarname');
                        var countEl = item.querySelector('.cnt_span');
                        ...
                        var a = item.querySelector('a');
                        var imgEl = item.querySelector('img');
                        var titleEl = item.querySelector('.catname');
                        var countEl = item.querySelector('.catnum');
                    ...
                    if (url && img && title) {
                        ...
                        if (isVideo) {
                            results.push({
                                ...
                                img: img,
                                time: time,
                                preview: previewUrl, // Передаємо посилання на webm-трейлер
                                is_models: false,
                                is_grid: false
                            });
                        } else {
                            results.push({
                                ...
                                picture: img,
                                img: img,
                                card_badge: badge, // Єдиний бейдж кількості
                                is_grid: true,
                                card_grid: isStudio ? 'categories-grid studios-grid' : 'categories-grid',
                                is_models: isModel,
                                is_categories: isCategory,
                                is_studios: isStudio
                            });
                        }
                    }
...
is_studios: isStudios,
                    ...
                    // --- БЛОК 2: СТАНДАРТНИЙ DOM ПАРСИНГ (ВІДЕО НА ГОЛОВНІЙ) ---
                    if (results.length === 0) {
                        ...
                        for (var j = 0; j < elements.length; j++) {
                            ...
                            // ПЕРЕНЕСЕНО З ОРИГІНАЛУ: Логіка hqporner та відносних шляхів
                ...
                getStreams: function(htmlText, doc, element, startPlayback, onError) {
                    ...
                    if (myDaddyMatch) {
                        for (var m = 0; m < myDaddyMatch.length; m++) {
                            var cleanUrl = myDaddyMatch[m];
                            if (mdUrls.indexOf(cleanUrl) === -1) {
                                mdUrls.push(cleanUrl);
                            }
                        }
                    }
                    ...
                    function tryNextProvider() {
                        ...
                        if (targetName === 'DIRECT') {
                            ...
                            startPlayback([{ title: bestQuality.title + ' (Direct)', url: bestQuality.url }]);
                            return;
                        }
                    ...
                    if (targetPath === '/categories' || object.is_categories) {
                        ...
                        for (var k = 0; k < mEls.length; k++) {
                            ...
                            if (linkM && nameM) {
                                ...
                                results.push({ 
    name: (nameM.textContent || '').trim(), 
    url: vUrlM, 
    picture: imgM ? imgM.getAttribute('src') : '', 
    img: imgM ? imgM.getAttribute('src') : '', 
    is_grid: true, 
    is_models: true,
    card_badge: countM ? (countM.textContent || '').trim() : '' // Кількість відео на плашку
});
    ...
    img: imgV, 
    preview: previewData,
    time: time // Тривалість на плашку
});
    ...
    is_grid: true, 
    is_models: true,
    card_badge: count // Кількість відео на плашку
});
    ...
    results.push({ 
        ...
        is_grid: isStudios, 
        preview: pUrl,
        time: isStudios ? '' : infoText, // Час для відео
        card_badge: isStudios ? '🎬 ' + infoText : '' // Кількість для студій
    });
}
                            }
                        }
                        ...
                        for (var m = 0; m < mEls.length; m++) {
                            ...
                            if (linkM && imgM) {
                                ...
                                if (rawName && urlM) {
                                    ...
                                    results.push({ name: rawName,
card_badge: countText, url: urlM, picture: imgSrcM, img: imgSrcM, is_grid: true, is_models: true });
                                }
                            }
                        }
                    } 
                        ...
                        for (var s = 0; s < sEls.length; s++) {
                            ...
                            if (linkS && imgS) {
                                ...
                                if (nameS && urlS) {
                                    ...
                                    results.push({ name: nameS,
card_badge: statsText, url: urlS, picture: imgSrcS, img: imgSrcS, is_grid: true });
                                }
                            }
                        }
                    }
...
time: timeText, url: href, picture: posterUrl, img: posterUrl });
                                }
                            }
                        }
                    }
                ...
                parse: function(doc, currentUrl, object) {
                    ...
                    for (var i = 0; i < elements.length; i++) {
                        ...
                        if (linkEl) {
                            ...
                            if (name && vUrl) results.push({ name: name,
time: time, url: vUrl, picture: img, img: img });
                        }
                    }
                    return results;
                },
                    ...
                    if (targetPath === '/categories' || targetPath === '/actors' || object.is_categories || object.is_models) {
                        ...
                        for (var v = 0; v < elements.length; v++) {
                            ...
                            if (linkV) {
                                ...
                                if (nameV && urlV) results.push({ name: nameV,
time: time, url: urlV, picture: imgV, img: imgV });
                            }
                        }
                    }
                    return results;
                ...
                if ($card && typeof $card.find === 'function') {
                    ...
                    if (badgeText) {
                        ...
                        if ($view.length > 0 && $view.find('.pluginx-time-badge').length === 0) {
                            var badgeHtml = '<div class="pluginx-time-badge" style="position: absolute; right: 7px; bottom: 7px; background: rgba(0,0,0,0.6); color: #fff; padding: 2px 6px; border-radius: 4px; font-size: 12px; font-weight: normal; font-family: inherit; border: none; z-index: 10; pointer-events: none;">' + badgeText + '</div>';
