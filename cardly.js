(function () {
    ...
    // --- Допоміжні функції Babel для класів (залишено для підтримки старих ТБ) ---
    ...
    var Player = (function () {
            ...
            if (typeof YT !== "undefined" && YT.Player) {
                this.youtube = new YT.Player(
                    this.html.find(".cardify-trailer__youtube-iframe")[0],
                    {
                        ...
                        playerVars: { controls: 1, showinfo: 0, autohide: 1, modestbranding: 1, autoplay: 0, disablekb: 1, fs: 0, enablejsapi: 1, playsinline: 1, rel: 0, suggestedQuality: "hd1080", setPlaybackQuality: "hd1080", mute: 1 },
                        videoId: video.id,
                        events: {
        ...
        _createClass(Player, [
            ...
            { key: "unmute", value: function unmute() { try { this.youtube.unMute(); this.html.find(".cardify-trailer__remote").remove(); window.cardify_fist_unmute = true; } catch (e) {} } },
            ...
            { key: "destroy", value: function destroy() { this.loaded = false; this.display = false; try { this.youtube.destroy(); } catch (e) {} clearInterval(this.timer); this.html.remove(); } }
        ]);
        return Player;
    })();
    ...
    var Trailer = (function () {
        ...
        _createClass(Trailer, [
            ...
            { key: "animate", value: function animate() { var _this2 = this; var loader = this.object.activity.render().find(".cardify-preview__loader").width(0); var started = Date.now(); clearInterval(this.timer_anim); this.timer_anim = setInterval(function () { var left = Date.now() - started; if (left > _this2.timelauch) clearInterval(_this2.timer_anim); loader.width(Math.round((left / _this2.timelauch) * 100) + "%"); }, 100); } },
            { key: "preview", value: function preview() { var preview = $('\n            <div class="cardify-preview">\n                <div>\n                    <img class="cardify-preview__img" />\n                    <div class="cardify-preview__line one"></div>\n                    <div class="cardify-preview__line two"></div>\n                    <div class="cardify-preview__loader"></div>\n                </div>\n            </div>\n        '); Lampa.Utils.imgLoad($("img", preview), this.video.img, function () { $("img", preview).addClass("loaded"); }); this.object.activity.render().find(".cardify__right").append(preview); } },
            { key: "controll", value: function controll() { var _this3 = this; var out = function out() { _this3.state.dispath("hide"); Lampa.Controller.toggle("full_start"); }; Lampa.Controller.add("cardify_trailer", { toggle: function toggle() { Lampa.Controller.clear(); }, enter: function enter() { _this3.player.unmute(); }, left: out.bind(this), up: out.bind(this), down: out.bind(this), right: out.bind(this), back: function back() { _this3.player.destroy(); _this3.object.activity.render().find(".cardify-preview").remove(); out(); } }); Lampa.Controller.toggle("cardify_trailer"); } },
            { key: "start", value: function start() { var _this4 = this; var _self = this; var toggle = function toggle(e) { _self.state.dispath("toggle"); }; var destroy = function destroy(e) { if (e.type == "destroy" && e.object.activity === _self.object.activity) remove(); }; var remove = function remove() { Lampa.Listener.remove("activity", destroy); Lampa.Controller.listener.remove("toggle", toggle); _self.destroy(); }; Lampa.Listener.follow("activity", destroy); Lampa.Controller.listener.follow("toggle", toggle); this.player = new Player(this.object, this.video); this.player.listener.follow("loaded", function () { _this4.preview(); _this4.state.start(); }); this.player.listener.follow("play", function () { clearTimeout(_this4.timer_show); if (!_this4.firstlauch) { _this4.firstlauch = true; _this4.timelauch = 5000; } _this4.timer_show = setTimeout(function () { _this4.player.show(); _this4.background.addClass("nodisplay"); _this4.startblock.addClass("nodisplay"); _this4.head.addClass("nodisplay"); _this4.controll(); }, 500); }); this.player.listener.follow("ended,error", function () { _this4.state.dispath("hide"); if (Lampa.Controller.enabled().name !== "full_start") Lampa.Controller.toggle("full_start"); _this4.object.activity.render().find(".cardify-preview").remove(); setTimeout(remove, 300); }); this.object.activity.render().find(".activity__body").prepend(this.player.render()); this.state.start(); } },
            { key: "destroy", value: function destroy() { clearTimeout(this.timer_load); clearTimeout(this.timer_show); clearInterval(this.timer_anim); this.player.destroy(); } }
        ]);
        return Trailer;
    })();

    // --- Спрощені функції обробки UI (колишній фейковий кеш) ---
    ...
    // --- Локалізація, Шаблони та Стилі ---
    function initTemplatesAndStyles() {
        Lampa.Lang.add({
            cardify_enable_sound: {
                ru: "Включить звук", en: "Enable sound", uk: "Увімкнути звук", be: "Уключыць гук", zh: "启用声音", pt: "Ativar som", bg: "Включване на звук"
            },
            cardify_enable_trailer: {
                ru: "Показывать трейлер", en: "Show trailer", uk: "Показувати трейлер", be: "Паказваць трэйлер", zh: "显示预告片", pt: "Mostrar trailer", bg: "Показване на трейлър"
            }
        });
    ...
    function startPlugin() {
        ...
        // Додаємо вкладку Cardify у Налаштування Lampa
        ...
        // Функція для пошуку найкращого трейлера з API
        function video(data) {
            if (data.videos && data.videos.results.length) {
                ...
                // Спочатку шукаємо трейлер мовою інтерфейсу Lampa
                var my_lang = items.filter(function (n) {
                    return n.code == Lampa.Storage.field("tmdb_lang");
                });
                
                // Потім англійською, виключаючи дублікати
        ...
        // --- Головний обробник подій (очищений від "шифрів" та фейкових кешів) ---
        Lampa.Listener.follow("full", function (e) {
            if (e.type == "complite") {
                // Застосовуємо UI-зміни (картку)
                skodf(e);
                
                // Перевіряємо чи увімкнуті фонові трейлери
                if (!Lampa.Storage.field("cardify_run_trailers")) return;
                
                var trailer = video(e.data);

                // Якщо Lampa підтримує об'єкт Manifest (зазвичай >= 220 версії)
                if (window.Lampa && window.Lampa.Manifest && window.Lampa.Manifest.app_digital >= 220) {
                    if (Lampa.Activity.active().activity === e.object.activity) {
                        trailer && new Trailer(e.object, trailer);
                    } else {
                        var followActivity = function followActivity(a) {
                            if (
                                a.type == "start" &&
                                a.object.activity === e.object.activity &&
                                !e.object.activity.trailer_ready
                            ) {
                                Lampa.Listener.remove("activity", followActivity);
                                trailer && new Trailer(e.object, trailer);
                            }
                        };
                        Lampa.Listener.follow("activity", followActivity);
                    }
                }
            }
        });
    ...
    // Запускаємо плагін одразу, якщо Lampa вже готова, або чекаємо на подію "ready"
    if (window.appready) {
        startPlugin();
    } else {
