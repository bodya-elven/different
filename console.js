(function () {
    ...
    // Легке оновлення сітки (тільки коли додалися/зникли елементи)
    ...
    function initUI() {
        ...
        // ШВИДКИЙ ТА ПЛАВНИЙ ЦЕНТРИЧНИЙ СКРОЛ
        function scrollToFocused() {
            ...
            if ($container.length) {
                ...
                // Використовуємо нативний скрол браузера замість jQuery animate для максимальної плавності
            ...
            if (window.Lampa && Lampa.Controller) {
                if (!window.lmc_controller_added) {
                    Lampa.Controller.add('lmc_console', {
                        ...
                        // МЕТОД ENTER ДЛЯ ПУЛЬТА (клікає або фокусує поле вводу)
                        enter: function() { 
                            var focused = window.Navigator.getFocusedElement();
                            if (focused) {
                                if (focused.id === 'lmc-search-input') {
                                    focused.focus();
                                } else {
        ...
        // РЕАКЦІЯ НА ЗАВЕРШЕННЯ РУХУ ПУЛЬТА (Страховка скролу)
        $('#lampa-mob-console-window').on('hover:focus', '.selector', function() {
            scrollToFocused();
        });

        // ЗАХИСТ BACKSPACE: Дозволяємо системі стерти літеру в пошуку
        ...
        $('#lampa-mob-console-window').on('click', '.selector', function(e) {
            ...
            // Розумне розгортання логів (тільки оновлення скролу, без collectionSet)
            if ($this.hasClass('lmc-row') || $this.hasClass('lmc-network-row') || $this.hasClass('lmc-item-text')) {
                ...
                if ($collapsible.length) {
                    ...
                    scrollToFocused(); // Просто підрівнюємо екран після зміни висоти
                }
                return false;
            }
        });
