(function () {
    ...
    function initUI() {
        ...
        var css = `
            #lampa-mob-console-window { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100vh; height: 100dvh; background: #0c0d0f; z-index: 9999999; flex-direction: column; font-family: sans-serif; color: #ddd; font-size: 12px; }
            #lampa-mob-console-header { display: flex; justify-content: space-between; padding: 10px; background: #1a1c1f; align-items: center; flex-shrink: 0; }
            #lmc-search-bar { padding: 8px; background: #141619; display: flex; align-items: center; flex-shrink: 0; }
            #lmc-search-input { flex: 1; background: #212429; color: #fff; border: 1px solid transparent; padding: 8px; border-radius: 4px; outline: none; }
            #lmc-search-input.focus { border-color: #20c997; }
            #lampa-mob-console-tabs { display: flex; background: #0c0d0f; overflow-x: auto; padding: 8px; flex-shrink: 0; }
            .lmc-tab { padding: 6px 12px; background: #16181b; border-radius: 6px; margin-right: 6px; color: #aaa; cursor: pointer; white-space: nowrap; }
            .lmc-tab.active { color: #20c997; border: 1px solid #20c997; }
            .lmc-tab.focus { background: #2a2e33; color: #fff; }
            .lmc-content { display: none; flex: 1; overflow-y: auto; padding: 10px; word-wrap: break-word; position: relative; }
            .lmc-content.active { display: flex; flex-direction: column; }
            #lampa-mob-console-close { font-size: 24px; cursor: pointer; padding: 0 10px; }
            #lampa-mob-console-close.focus { color: #f44336; }
            .lmc-row { padding: 8px; border-bottom: 1px solid rgba(255,255,255,0.05); }
            .lmc-row.focus { background: rgba(32, 201, 151, 0.1); }
        ...
        $('body').append(`
            <div id="lampa-mob-console-window">
                ...
                <div id="lampa-mob-console-tabs">
                    <div class="lmc-tab selector active" data-target="lmc-content-logs">Console</div>
                    <div class="lmc-tab selector" data-target="lmc-content-errors">Errors</div>
                    <div class="lmc-tab selector" data-target="lmc-content-network">Network</div>
        ...
        function scrollToFocused() {
            ...
            if ($container.length) {
                ...
                if (elTop < contScroll || elTop + elHeight > contScroll + contHeight) {
                    $container[0].scrollTop = elTop - (contHeight / 2);
                }
            }
        }
        ...
        function openConsole() {
            if (window.Lampa && Lampa.Controller) {
                ...
                if (!window.lmc_controller_added) {
                    Lampa.Controller.add('lmc_console', {
                        toggle: function() {
                            ...
                            Lampa.Controller.collectionFocus(false, document.getElementById('lampa-mob-console-window'));
                        },
                        ...
                        down:  function() { 
                            ...
                            if (f && f.id === 'lmc-search-input') window.Navigator.focus(document.querySelector('.lmc-tab.active'));
                            else window.Navigator.move('down'); 
                            scrollToFocused();
                        },
                        up:    function() { 
                            var f = window.Navigator.getFocusedElement();
                            if (f && f.classList.contains('lmc-tab')) window.Navigator.focus(document.getElementById('lmc-search-input'));
                            else window.Navigator.move('up'); 
                            scrollToFocused();
                        },
                        enter: function() { var f = window.Navigator.getFocusedElement(); if (f) $(f).trigger('click'); },
                ...
                window.Navigator.focus(document.querySelector('.lmc-tab.active'));
            }
        }
